...
from random import choice, randint, sample, shuffle, uniform, random
...
def create_arguments(number_agents, maximum_number_practical_arguments, maximum_number_epistemic_arguments,
		 attacks_density_value, alternatives):
	...
	for i in range(number_agents):
		...
		all_ep = all_ep + epistemic_arguments_base
		all_pr = all_pr + practical_arguments_base 

		arguments_bases.append([practical_arguments_base, epistemic_arguments_base])

	all_attacks = [(ep, pr) for ep in all_ep for pr in all_pr]
	le = len(all_ep)
	for i in range(le):
		for j in range(le):
			if i != j:
	...
	attacks_number = round(len(all_attacks) * attacks_density_value, 0)
	...
	return arguments_bases, all_ep, all_pr, attacks

def create_agents_bases(alternatives, arguments_bases, all_ep, all_pr, attacks, redundancy):
	bases = []
	alternatives_list = []
	alternatives_number = len(alternatives)
	alternatives_l = list(alternatives)

	if redundancy > 0:
		for i in range(len(arguments_bases)):
			for ep in all_ep:
				if random() < redundancy:
					if ep not in arguments_bases[i][1]: # EPIST
						arguments_bases[i][1].append(ep)
			for pr in all_pr:
				if random() < redundancy:
					if pr not in arguments_bases[i][0]: # PRAC
						arguments_bases[i][0].append(pr)

		for i in range(len(arguments_bases)):
			practical_arguments_number = len(arguments_bases[i][0]) # PRAC
			if alternatives_number < practical_arguments_number:
				alternatives_list = alternatives_l + [choice(alternatives_l) for j in range(practical_arguments_number - alternatives_number)]	
			
			practical_base = []
			index = 0
			for practical_argument_label in arguments_bases[i][0]: # PRAC
				type_label = choice(["P", "C"])
				certainty_value = uniform(0, 1)
				practical_argument_structure = (practical_argument_label, type_label, alternatives_list[index], [], certainty_value, [], [])
				practical_base.append(practical_argument_structure)
				index = index + 1
				
			epistemic_base = []
			for epistemic_argument_label in arguments_bases[i][1]: # EPIS
				epistemic_argument_structure = (epistemic_argument_label, [], [], [])
				epistemic_base.append(epistemic_argument_structure)
			
			att = [at for at in attacks if random() < redundancy]

			for at in att:
				if at[0] in arguments_bases[i][1]:
					for k in range(len(epistemic_base)):
						if at[0] == epistemic_base[k][0]:
							epistemic_base[k][3].append(at[1])
							break
				if at[1] in arguments_bases[i][1]:
					for k in range(len(epistemic_base)):
						if at[1] == epistemic_base[k][0]:
							epistemic_base[k][1].append(at[0])
							break
				if at[1] in arguments_bases[i][0]:
					for k in range(len(practical_base)):
						if at[1] == practical_base[k][0]:
							practical_base[k][3].append(at[0])
							break
			bases.append([practical_base, epistemic_base])
	else:
		for i in range(len(arguments_bases)):
			practical_arguments_number = len(arguments_bases[i][0])
			if alternatives_number < practical_arguments_number:
				alternatives_list = alternatives_l + [choice(alternatives_l) for j in range(practical_arguments_number - alternatives_number)]	
				
			practical_base = []
			index = 0
			for practical_argument_label in arguments_bases[i][0]:
				type_label = choice(["P", "C"])
				certainty_value = uniform(0, 1)
				practical_argument_structure = (practical_argument_label, type_label, alternatives_list[index], [], certainty_value, [], [])
				practical_base.append(practical_argument_structure)
			...
			epistemic_base = []
			for epistemic_argument_label in arguments_bases[i][1]:
				epistemic_argument_structure = (epistemic_argument_label, [], [], [])
				epistemic_base.append(epistemic_argument_structure)
				
			for at in attacks:
				if at[0] in arguments_bases[i][1] and at[1] in arguments_bases[i][0]:
					for k in range(len(epistemic_base)):
						if at[0] == epistemic_base[k][0]:
							epistemic_base[k][3].append(at[1])
							break
				...
				if at[1] in arguments_bases[i][1] and at[0] in arguments_bases[i][1]:
					for k in range(len(epistemic_base)):
						if at[1] == epistemic_base[k][0]:
							epistemic_base[k][1].append(at[0])
							break
...
def create_agents(number_agents, alternatives_number, maximum_number_practical_arguments,
 maximum_number_epistemic_arguments, maximum_attacks_density_value, redundancy):
	...
	if number_agents >= 1 and alternatives_number >= 2 and isinstance(alternatives_number, int) and maximum_attacks_density_value >= 0 and maximum_attacks_density_value <= 1 and maximum_number_epistemic_arguments >= 1 and maximum_number_practical_arguments >= alternatives_number:
		...
		args_bs_l, all_ep, all_pr, attacks  = create_arguments(number_agents,
										 maximum_number_practical_arguments,
										 maximum_number_epistemic_arguments,
										 maximum_attacks_density_value,
										 alternatives)
		
		arguments = create_agents_bases(alternatives, args_bs_l, all_ep, all_pr, attacks, redundancy)
		...
		return agents, all_ep, all_pr, attacks
